...
async function handleLogin() {
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (res.code === 200) {
        userStore.setToken(res.data.token); userStore.setUserInfo({ nickname: res.data.nickname })
        await userStore.syncFamilyInfo(); ElMessage.success('登录成功'); router.push('/')
...
.login-page {
  ...
  background-image: radial-gradient(circle, rgba(58, 52, 40, 0.12) 1px, transparent 1px);
  background-size: 20px 20px; background-position: 2px 2px;
}
.paper-card {
