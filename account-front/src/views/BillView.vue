<template>
  <div class="bill-container">
    ...
    <el-tabs v-model="activeTab" class="mb-4" @tab-change="handleTabChange">
      <el-tab-pane label="📋 我的记录" name="my" />
      <el-tab-pane label="👨‍👩‍👧 家人记录" name="family" />
    </el-tabs>
      ...
      <el-row :gutter="20" align="middle">
        ...
        <el-col :span="5">
          <div class="amount-range" style="display: flex; align-items: center;">
            <el-input v-model="filters.minAmount" type="number" placeholder="最小金额" style="width: 42%" />
            <span style="margin: 0 8px;">~</span>
            <el-input v-model="filters.maxAmount" type="number" placeholder="最大金额" style="width: 42%" />
          </div>
        </el-col>
    ...
    <el-card>
      <el-table :data="billList" stripe fit>
        ...
        <el-table-column label="可见范围" align="center" header-align="center">
          <template #default="{ row }">
            <el-tag :type="row.visible === 'FAMILY' ? 'info' : ''" class="visibility-tag">
    ...
    <!-- 导入弹窗 -->
    <el-dialog v-model="importDialogVisible" title="导入收支记录" width="650px">
      <el-form :model="importForm" label-width="100px">
        ...
        <el-form-item label="上传文件" required>
          ...
          >
            ...
            <template #tip>
              <div class="el-upload__tip">支持 .xlsx 和 .csv 格式文件</div>
            </template>
          </el-upload>
        </el-form-item>
      </el-form>
      ...
      <div class="import-help">
        ...
        <div v-if="showHelp" style="margin-top: 15px;">
          <h4 style="margin-bottom: 10px;"><span class="iconfont icon-daorugeshi_upload" style="font-size: 16px;"></span> 导入格式说明</h4>
          <p style="font-size: 13px; color: #666; margin-bottom: 10px;">
            <strong>列顺序（必须严格按照此顺序）：</strong><br>
            1. 日期（必填）- 格式：YYYY-MM-DD<br>
            2. 类型（必填）- INCOME（收入）/ EXPENSE（支出）<br>
            3. 分类名称（必填）- 需与系统分类一致<br>
            4. 金额（必填）- 必须大于0<br>
            5. 可见范围（选填）- PRIVATE（仅自己可见）/ FAMILY（家庭成员可见），默认PRIVATE<br>
            6. 备注（选填）- 描述信息
          </p>
          
          <h4 style="margin-bottom: 10px;"><span class="iconfont icon-xiazaishilishuju" style="font-size: 16px;"></span> 下载示例数据</h4>
          <div style="max-height: 180px; overflow-y: auto; border: 1px solid #eee; border-radius: 4px;">
            <table style="width: 100%; font-size: 12px; border-collapse: collapse;">
              <thead>
                <tr style="background: #f5f7fa;">
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">日期</th>
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">类型</th>
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">分类名称</th>
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">金额</th>
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">可见范围</th>
                  <th style="border: 1px solid #eee; padding: 6px; text-align: center;">备注</th>
                </tr>
              </thead>
              <tbody>
                <tr><td style="border: 1px solid #eee; padding: 4px; text-align: center;">2026-05-26</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">EXPENSE</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">餐饮</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">25.50</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">PRIVATE</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">午餐外卖</td></tr>
                <tr><td style="border: 1px solid #eee; padding: 4px; text-align: center;">2026-05-25</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">INCOME</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">工资</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">12000.00</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">FAMILY</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">五月工资</td></tr>
                <tr><td style="border: 1px solid #eee; padding: 4px; text-align: center;">2026-05-24</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">EXPENSE</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">购物</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">156.00</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">PRIVATE</td><td style="border: 1px solid #eee; padding: 4px; text-align: center;">日用品</td></tr>
              </tbody>
            </table>
          </div>
          
          <h4 style="margin-bottom: 10px; margin-top: 10px;"><span class="iconfont icon-fenlei" style="font-size: 16px;"></span> 分类</h4>
          <div style="font-size: 12px; color: #666; columns: 2; gap: 20px;">
            <div>
              <strong>支出：</strong><br>
...
import { getBillList, addBill, updateBill, deleteBill, getFamilyBillList } from '@/api/bill'
...
// 导入URL
const importUrl = computed(() => `${import.meta.env.VITE_API_BASE_URL}/bill/import`)

// 打开导入对话框
const openImportDialog = () => {
  importResult.value = null
  importForm.value = { source: 'SYSTEM' }
...
const handleImportSuccess = (response) => {
  if (response.code === 200) {
    const { successCount, failCount, failReasons } = response.data
    let msg = `导入完成！成功 ${successCount} 条`
    if (failCount > 0) {
      msg += `，失败 ${failCount} 条\n\n失败原因：\n`
      failReasons.forEach(item => {
        msg += `第 ${item.row} 行: ${item.reason}\n`
      })
    }
    ...
    ElMessageBox.alert(response.message || '导入失败', '导入失败', {
      confirmButtonText: '确定',
      type: 'error'
    })
  }
